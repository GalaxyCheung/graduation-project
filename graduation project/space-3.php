...
<?php
	...
	class Space3 {
		...
		function queryFollowUser(){
			...
			$sql = "select * from gp_user_follow inner join gp_user on gp_user_follow.f_id=gp_user.id where gp_user_follow.u_id='$this->userId' ORDER BY gp_user_follow.id DESC LIMIT $this->startRow,$this->pageSize";
			$result = mysqli_query($link,$sql);
			while($rs = mysqli_fetch_array($result)){
				...
				if(mysqli_num_rows($result) < 1){
					echo "<div style='margin:80px; 80px;'><h1>这个人很懒，谁也没关注</h1></div>";
				}
			mysqli_free_result($result);
			mysqli_close($link);
		}	
			...
			<?php
				...
				if($curPage<=0||$curPage==""){
					...
					if($curPage!=1){
						if($curPage<=3){
							for ($i=1; $i<$curPage; $i++){
								echo "<a href='space-3.php?id=$userId&page=$i'>".$i."</a>";
							}
						}else{
							echo "<a href='space-3.php?page=".($curPage-1)."'>上一页&nbsp;&gt;</a>
							<a href='space-3.php?id=$userId&page=1'>1</a>
								<a>...</a>";
				...
				<?php
					...
					if(($pageNum-$curPage)>=3){
						echo "<a>...</a>
						<a href='space-3.php?id=$userId&page=".($curPage+1)."'>下一页&nbsp;&gt;</a>";
					}
			?>
		</div>
			<div class="clear"></div>
